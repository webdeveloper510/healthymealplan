...
class GroceriesGrid extends React.Component {
  constructor(props) {
    super(props);
    ...
    return (
      <div style={{ width: '100%' }}>
              ...
              (
                <Grid item xs={12} sm={6} md={4} lg={4} style={{ minWidth: '320px' }} key={i}>
                  <Card style={{ width: '100%' }}>
                    ...
                    <CardActions>
                      <Button dense color="primary" onClick={() => this.props.history.push(`/groceries/${e._id}/edit`)}>Edit</Button>
                    </CardActions>
                  </Card>
        ...
        <Typography className="font-medium" type="body2" style={{ marginTop: '25px', color: 'rgba(0, 0, 0, .54)' }}>
          Showing {this.props.count} of {this.props.plateCount} groceries
        </Typography>
...
export default createContainer(() => {
  const plateCountSub = Meteor.subscribe('groceries-all-count');

  return {
    plateCount: Counts.get('groceries'),
  };
}, GroceriesGrid);
